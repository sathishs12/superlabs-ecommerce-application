...
const ProductPage: React.FC = () => {
 ...
 const images: string[] = product.productImages?.reduce((acc, img) => {
   if (img.image) {
     acc.push(getMediaAssetUrl(img.image));
     return acc;
   } else acc.push(fallbackImage);
   return acc;
 }, [] as string[]) ?? [fallbackImage];
       ...
       <Grid container spacing={{ xs: 4, sm: 5, md: 6 }}>
         ...
         <Grid item xs={12} lg={6}>
           <Box sx={{ position: "relative" }}>
             ...
             {images.length > 1 && (
               <Box
                 sx={{ display: "flex", gap: 1.5, overflowX: "auto", py: 1 }}
               >
                 {images.map((img: string, idx: number) => (
                   <Box
                     ...
                     sx={{
                       ...
                       border: 2,
                       borderColor:
                         selectedImage === idx ? "black" : "grey.200",
                       cursor: "pointer",
                       transition: "border-color 0.2s",
                       "&:hover": {
                         borderColor:
                           selectedImage === idx ? "black" : "grey.400",
                       },
                     }}
                   >
                     <Box
